...
async function getArticle(slug: string, preview = false) {
  // Preview: service client, no status filter, so an admin can see a DRAFT.
  // The page gates this with requireAdmin() before calling it, so drafts stay
  // private to admins.
  if (preview) {
    const admin = createServiceClient();
    const { data } = await admin.from("articles").select("*").eq("slug", slug).maybeSingle();
    return data;
  }
...
export default async function ArticlePage({
  ...
  // Only admins can preview an unpublished draft; requireAdmin redirects others.
  ...
  return (
    <main className="min-h-screen bg-background flex flex-col font-sans">
      ...
      {preview && (
        <div className="bg-amber-500 text-amber-950 text-sm font-semibold text-center px-4 py-2">
          Preview · status: {article.status} — this is not published yet. Only you can see this.
        </div>
      )}
